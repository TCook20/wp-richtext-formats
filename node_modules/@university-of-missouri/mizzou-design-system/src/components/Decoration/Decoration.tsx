...
export interface DecorationTearProps extends ElementAttributes {
  theme?: 'light' | 'dark'
}

const DecorationTear = ({
  attributes = new DrupalAttribute(''),
}: DecorationTearProps): JSX.Element => {
  const [containerHeight, setContainerHeight] = React.useState(0)
  const [containerWidth, setContainerWidth] = React.useState(0)
  const [tearHeight, setTearHeight] = React.useState(0)

  const measureContainer = React.useCallback((node) => {
    if (node !== null) {
  ...
  const measureTear = React.useCallback((node) => {
    if (node !== null) {
      setTearHeight(node.getBoundingClientRect().height)
    }
  }, [])

  const tearTranslate = containerHeight - tearHeight

  const DecorationTearAttributes = {
    id: 'tearSVG',
    ref: measureContainer,
    xmlns: 'http://www.w3.org/2000/svg',
    className: classNames('miz-tear', attributes.get('class')),
  }

  const TearPathAttributes = {
    id: 'tearPath',
    ref: measureTear,
    width: containerWidth,
    transform: `matrix(1, 0, 0, 1, 0, ${tearTranslate})`,
    d:
  ...
  return (
    <svg {...DecorationTearAttributes}>
      <defs>
        <clipPath id="tearClipPath">
          <path {...TearPathAttributes} />
          <rect {...RectAttributes} />
        </clipPath>
      </defs>
    </svg>
